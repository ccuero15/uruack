                ...
                <form action="{{ isset($cargo) ? route('cargos.update', $cargo) : route('cargos.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="titulo" value="{{ old('titulo', $cargo->titulo ?? '') }}" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Departamento</label>
                            <input type="text" name="departamento" value="{{ old('departamento', $cargo->departamento ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative mt-1">
                                ...
                                <input type="number" step="0.01" name="salario_referencial" value="{{ old('salario_referencial', $cargo->salario_referencial ?? '') }}"
                    ...
                    <div class="mt-8 flex justify-end gap-3">
                        <a href="{{ route('cargos.index') }}" class="text-gray-600 px-4 py-2 hover:underline">Cancelar</a>
