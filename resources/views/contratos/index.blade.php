                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    @if (in_array(auth()->user()->rol_id, [1, 2]))
                        <a href="{{ route('contratos.create') }}"
                            class="bg-blue-600 text-white px-4 py-2 rounded text-sm">+ Nuevo Contrato</a>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($contratos as $c)
                            <tr>
                                <td class="px-6 py-4 text-sm">{{ $c->empleado->nombre }} {{ $c->empleado->apellido }}
                                </td>
                                <td class="px-6 py-4 text-sm">{{ $c->cargo->titulo }}</td>
                                <td class="px-6 py-4 text-sm font-bold">${{ number_format($c->salario_base, 2) }}</td>
                                <td class="px-6 py-4 text-sm">
                                    <span
                                        class="px-2 py-1 {{ $c->estado == 'Vigente' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} rounded-full text-xs font-bold">
                                        {{ $c->estado }}
                                    </span>
                                </td>
                            </tr>
